...
import { baseUrl, payBaseUrl } from 'src/environments/environment';
...
export class ServicesService {
  ...
  public getUser(token: any) {
    let tokenStr = token;
    let headers = new HttpHeaders().set('Authorization', tokenStr);
    let resp = this.httpClient.get(`${baseUrl}/api/v1/user/getUser`, {
      headers: headers,
    });
    return resp;
  }
  ...
  public assignRole(role: Role, userId: string, token: any) {
    ...
    let resp = this.httpClient
      .post(`${baseUrl}/api/v1/assignRole?userId=${userId}`, role, {
        headers: headers,
      })
      .pipe(
        tap(() => {
